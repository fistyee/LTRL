...
def learing_rate_scheduler(optimizer, config):
    if "type" in config._config["lr_scheduler"]: 
        if config["lr_scheduler"]["type"] == "CustomLR": # linear learning rate decay
            ...
            def lr_lambda(epoch):
                if lr_scheduler_args.get('step3') is not None and epoch >= lr_scheduler_args['step3']:
                    lr = gamma * gamma * gamma
                elif epoch >= lr_scheduler_args.get("step2", float('inf')):  # 默认值设为无穷大，确保跳过
                    lr = gamma * gamma
                elif epoch >= lr_scheduler_args.get("step1", float('inf')):
                    lr = gamma
                else:
                    lr = 1
