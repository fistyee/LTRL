...
from .imbalance_cifar import IMBALANCECIFAR10, IMBALANCECIFAR100, GENCIFAR10
...
class ImbalanceCIFAR100DataLoader(DataLoader):
    ...
    """
    def __init__(self, data_dir, batch_size, shuffle=True, num_workers=1, training=True, balanced=False, retain_epoch_size=True, imb_type='exp', imb_factor=0.01, rand=True):
        ...
        train_trsfm_rand = transforms.Compose([
        ...
        if training:
            if rand:
                dataset = IMBALANCECIFAR100(data_dir, train=True, download=True, transform=train_trsfm_rand, imb_type=imb_type, imb_factor=imb_factor)
            else:
...
class GENCIFAR10DataLoader(DataLoader):
    """
    Imbalance Cifar10 Data Loader
    """
    def __init__(self, data_dir, batch_size, shuffle=True, num_workers=1, training=True, balanced=False, retain_epoch_size=True, imb_factor=0.01):
        ...
        if training:
            dataset = GENCIFAR10(data_dir, train=True, download=True, transform=train_trsfm, imb_factor=0.01)
            val_dataset = datasets.CIFAR10(data_dir, train=False, download=True, transform=test_trsfm) # test set
        else:
            dataset = datasets.CIFAR10(data_dir, train=False, download=True, transform=test_trsfm) # test set
            val_dataset = None
